...
namespace ProjectDawn.Navigation.Hybrid
{
    ...
    public class AgentAvoidAuthoring : MonoBehaviour
    {
        ...
        void Awake()
        {
            ...
            if (UseWalls)
            {
                if (!gameObject.TryGetComponent<INavMeshWallProvider>(out var _))
        ...
        class AgentSonarAvoidBaker : Baker<AgentAvoidAuthoring>
        {
            public override void Bake(AgentAvoidAuthoring authoring)
            {
                var entity = GetEntity(TransformUsageFlags.Dynamic);
                AddComponent(entity, authoring.DefaultAvoid);
                if (authoring.UseWalls)
                {
                    AddComponent(entity, new NavMeshBoundary { Radius = authoring.Radius + 1 });
                    AddBuffer<NavMeshWall>(entity);
                }
            }
        }
    }
